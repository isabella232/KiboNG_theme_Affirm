define([
    ...
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext, AffirmPay) {
        ...
        var CheckoutStep = Backbone.MozuModel.extend({
            ...
            isNonMozuCheckout: function() {
                ...
                if (activePayments && activePayments.length === 0){
                    return this.isAffirmPayment();
                }
                return ( activePayments && this.isAffirmPayment() );
            },
            isAffirmPayment: function() {
                return ( this.getOrder().get('billingInfo').get('paymentType')  == 'PayWithAffirm' );
            },
        ...
        BillingInfo = CheckoutStep.extend({
            ...
            relations: {
				...
				paywithaffirm: PaymentMethods.PayWithAffirm
            },
            ...
            affirmPaymentSelected: function() {
                return this.get('paymentWorkflow') === 'PayWithAffirm';
            },
            ...
            selectPaymentType: function(me, newPaymentType) {
				if(newPaymentType === 'PayWithAffirm'){
                    me.set('paymentWorkflow', 'PayWithAffirm');
                    me.set('usingSavedCard', false);
                }
                else{
                    me.unset( 'paymentWorkflow' );
                }
				...
				me.get('paywithaffirm').selected = newPaymentType === 'PayWithAffirm';
            ...
            hasPaymentChanged: function(payment) {
                ...
                function normalizeBillingInfos(obj) {
                    return {
                        ...
                        check: obj.check || {},
						paywithaffirm: obj.paywithaffirm || {}
                    };
                }
            ...
            applyPayment: function () {
                ...
                else if ( this.get("paymentType") == "PayWithAffirm" )
                    this.set("paymentWorkflow", 'PayWithAffirm');
                this.syncApiModel();
                if (this.nonStoreCreditTotal() > 0) {
					if ( this.get("paymentType") == "PayWithAffirm" ){
...
isNonMozuCheckout: function() {
                ...
                return (activePayments && (_.findWhere(activePayments, { paymentType: 'PayWithAmazon' }) || _.findWhere(activePayments, { paymentType: 'PayPalExpress2' }) || _.findWhere(activePayments, { paymentType: 'PayWithAffirm' }) ));
            },
			isAffirmPayment: function() {
                return ( this.get('billingInfo').get('paymentType')  == 'PayWithAffirm' );
            },
            buildAffirmData: function(){
                var affirmProps = this.get('affirmProps');
                var orderInfo = this;
                ...
                var metaData = {
                    'order_id': orderInfo.get('id'),
                    'platform_type': 'Kibo NG',
                    'platform_version': '5.0',
                    'platform_affirm': '1.0'
                };
                var configData =  {
                    'financial_product_key' : affirmProps.financialApiKey
                 };
                var merchantData = {
                ...
                var billingData  = {
                    'name': {
                        'first': billingContact.get('firstName'),
                        'last':billingContact.get('lastNameOrSurname')
                    },
                    'address': {
                        'line1' : billingContact.get('address').get('address1'),
                        'city' : billingContact.get('address').get('cityOrTown'),
                        'state' : billingContact.get('address').get('stateOrProvince'),
                        'zipcode': billingContact.get('address').get('postalOrZipCode')
                    },
                    'email' : billingContact.get('email'),
                    'phone_number' : billingContact.get('phoneNumbers').get('home')
                };
                ...
                if(billingInfo.get('isSameBillingShippingAddress')) {
                    ...
                    shippingData = {
                        'name': {
                            'first': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'firstName' ),
                            'last': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'lastNameOrSurname' )
                        },
                        'address': {
                            'line1': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'address' ).get( 'address1' ),
                            'city': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'address' ).get( 'cityOrTown' ),
                            'state': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'address' ).get( 'stateOrProvince' ),
                            'zipcode': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'address' ).get( 'postalOrZipCode' )
                        },
                        'email': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'email' ),
                        'phone_number': orderInfo.get('fulfillmentInfo').get('fulfillmentContact').get( 'phoneNumbers' ).get( 'home' )
                    };
                }
                var itemsData = [];
                orderInfo.get( 'items' ).forEach(function(item) {
                    var imageURL = item.product.imageUrl;
                    if( imageURL && imageURL.indexOf( 'http' ) < 0 ){
                        if( imageURL.indexOf( '//' ) >= 0 ){
                            imageURL = urlParts[0] + item.product.imageUrl;
                        }
                        else{
                            imageURL = urlParts[0] + "//" + item.product.imageUrl;
                        }
                ...
                return {
                    'metadata' : metaData,
                    'config' : configData,
                    'financingProgram': affirmProps.financingProgram,
            ...
            submit: function () {
				if( this.isAffirmPayment() ){
                    window.affirm.checkout( this.buildAffirmData() );
                }
                ...
                if( this.isAffirmPayment() ){
                    this.isSubmitting = false;
                    api.steps(process).then( window.affirm.checkout.post() );
                }
                else{
